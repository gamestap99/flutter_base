...
typedef MainMessageHandler = FutureOr Function(dynamic data, SendPort isolateSendPort);
typedef IsolateMessageHandler = FutureOr Function(dynamic data, SendPort mainSendPort, SendErrorFunction onSendError);
...
class Worker {
  ...
  static Future<void> _isolateInitializer(
    ...
    await for (var data in isolateReceiverPort) {
      final handlerFuture = params.isolateHandler(data, params.mainSendPort, params.errorSendPort?.send ?? (_) {});
      if (params.queueMode) {
        await handlerFuture;
      }
    }
