...
class Parallel {
  ...
  ///
  /// ```
  static FutureOr<R?> run<T, R>(ParallelCallback<T, R> handler, {T? entryValue}) async {
  ...
  ///
  /// ```
  static FutureOr<List<R>> map<T, R>(List<T> values, FutureOr<R> Function(T item) handler) async {
    final completerList = Map.fromIterables(values, values.map((e) => Completer()));
  ...
  ///
  /// ```
  static FutureOr<void> foreach<T>(List<T> values, FutureOr<void> Function(T item) handler) async {
    final completerList = Map.fromIterables(values, values.map((e) => Completer()));
  ...
  static void _isolateHandler(event, SendPort mainSendPort, SendErrorFunction? sendError) async {
