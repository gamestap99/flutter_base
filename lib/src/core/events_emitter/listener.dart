import 'package:flutter_base/src/core/events_emitter/emitters/event_emitter.dart';
import 'package:flutter_base/src/core/events_emitter/events_emitter.dart';
import 'package:flutter_base/src/core/events_emitter/utils/type.dart';
...
class EventListener<CallbackDataT> {
  ...
  bool validate<T extends Event>(T event) => (event is CallbackDataT || event.data is CallbackDataT || isSubtype<T, CallbackDataT>() || isSubtype<T, Event<CallbackDataT>>()) && (event.type == type || type == null);

  /// Checks if the listener matches the **event type**, **data type** and **callback**.
  bool matches<T>([String? type, EventCallback<T>? callback]) => (isSubtype<CallbackDataT, T>() || isSubtype<Event<CallbackDataT>, T>()) && (this.type == type || this.type == null || type == null) && (this.callback == callback || callback == null);
