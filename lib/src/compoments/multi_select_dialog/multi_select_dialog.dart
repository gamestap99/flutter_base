...
class _MultiSelectDialogState<T> extends State<MultiSelectDialog<T>> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
          ...
          : Row(
              ...
              children: <Widget>[
                _showSearch
                    ...
                    : widget.title ?? const Text("Select"),
                IconButton(
                  icon: _showSearch ? widget.closeSearchIcon ?? const Icon(Icons.close) : widget.searchIcon ?? const Icon(Icons.search),
                  onPressed: () {
                    setState(() {
      ...
      contentPadding: widget.listType == null || widget.listType == MultiSelectListType.LIST ? const EdgeInsets.only(top: 12.0) : const EdgeInsets.all(20),
      content: SizedBox(
        height: widget.height,
        width: widget.width ?? MediaQuery.of(context).size.width * 0.73,
        child: widget.customBuilder?.call((item) => _buildListItem(item.call(_items))) ??
            (widget.loading
                ? const Center(
